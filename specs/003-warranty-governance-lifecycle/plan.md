...
|---|---|---|---|---|
| **DEC-031** | Warranty Parts Expense | Absorbed into ticket repair cost (`repair.router.ts#L663`) | Post discrete debit to `acc-5040` (code `5040`, `Warranty Parts Expense`) and credit `acc-1040` (code `1040`, `Spare Parts Inventory`) on warranty ticket completion | `repair.service.ts`, `journal_entries` |
...
- **External Durability (Option a):** `server/uploads/` directory joins automated external USB mirroring (`USB_BACKUP_PATH/uploads`) during backup operations so warranty forensic evidence survives host drive failure.
...
| **TASK-3.4** | `P0` | **Warranty Parts Operating Expense Tracking (`DEC-031`, `FR-009`)**<br>Debit `acc-5040` (code `5040`, `Warranty Parts Expense`) and credit `acc-1040` (code `1040`, `Spare Parts Inventory`) when parts are consumed on a warranty ticket. Ensure ticket invoice total remains 0 piastres for customer. Add Test Suite 74. | TASK-3.2 | 1.5h | Test Suite 74: Warranty repair consuming 500 EGP screen writes 0 piastres invoice to customer, generates journal entry lines debiting acc-5040 and crediting acc-1040. |
| **TASK-3.5** | `P1` | **Rejected Supplier Returns to `DEFECTIVE_SCRAP` & Liquidation Gate (`DEC-035`, `FR-010`)**<br>In `procurement.router.ts`, add RTV rejection endpoint `POST /api/procurement/rtv/:id/reject` to transition items to `status = 'DEFECTIVE_SCRAP'`. Reject POS sale of scrap with HTTP 409. Add manager salvage sale endpoint `POST /api/inventory/scrap/liquidate`. Add Test Suite 75. | None | 1.0h | Test Suite 75: (1) RTV rejection marks item DEFECTIVE_SCRAP, (2) POS checkout rejected with 409, (3) Manager liquidation authorizes sale with audit log. |
