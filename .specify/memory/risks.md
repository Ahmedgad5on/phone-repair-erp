...
| **RISK-004** | High | Unauthorized financial mutation, cashier discount manipulation, or voided sale without audit justification | Enforce synchronous append-only audit trail in `audit_log` with HTTP 400 Bad Request on empty reason; role-based access control (RBAC) restricting voids, scrap liquidation, and price edits to MANAGER/ADMIN. Mitigated per risks.md lifecycle: evidenced by verbatim passing output in Test Suites 62, 71, 73, 74, and 75. | **Mitigated** | `PROJECT.md#L39` |
